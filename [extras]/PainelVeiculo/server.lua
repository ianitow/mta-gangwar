...
addEventHandler("onVehicleExplode", root, function()
	setVehicleHandling(source, "suspensionLowerLimit", nil)
end)
...
function Susp1()
	...
	if not(getVehicleHandling(vehicle)['suspensionLowerLimit'] >= -0.5) then return end
    setVehicleHandling ( vehicle, "suspensionLowerLimit", (getVehicleHandling(vehicle)['suspensionLowerLimit'])-0.1 )
end
addEvent("subir", true)
addEventHandler("subir", root, Susp1)
...
addEventHandler("descer", root, Susp2)
 
 
function Susp3()
    local vehicle = getPedOccupiedVehicle(source)
    if not vehicle then return end
    setVehicleHandling ( vehicle, "suspensionLowerLimit", nil )
end
addEvent("zerar", true)
addEventHandler("zerar", root, Susp3)
...
addEvent("farolon", true)
addEventHandler("farolon", root, farolon)

function faroloff()
	local vehicle = getPedOccupiedVehicle(source)
	if not vehicle then return end
    setVehicleOverrideLights ( vehicle, 1 )            -- force the lights off
end
addEvent("faroloff", true)
addEventHandler("faroloff", root, faroloff)
...
addEventHandler("motoron", root, motoron)
...
addEventHandler("motoroff", root, motoroff)
-------Motor------