...
function doDrawScoreboard( rtPass, onlyAnim, sX, sY )
	...
	if #scoreboardColumns ~= 0 then
				...
				for key, team in ipairs( teams ) do
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.name == "name" then
							...
							local teamXP = getElementData(team,"xp") or "SEM"
							teamName = teamName .. " (" .. teamMemberCount .. " player" .. iif( teamMemberCount == "1", "", "s" ) .. ") // " ..teamXP.." XP"							if serverInfo.allowcolorcodes then
					...
					for key, player in ipairs( players ) do
						...
						for key, column in ipairs( scoreboardColumns ) do
							...
							if column.name == "name" then
								local teamTag = ""
								if getElementData(team,"tag") then
									teamTag = "["..getElementData(team,"tag").."]"
								else
									teamTag = ""
								end
								
								local playerName = teamTag..getPlayerName( player )
								if serverInfo.allowcolorcodes then
									if string.find( playerName, "#%x%x%x%x%x%x" ) then
										local colorCodes = {}
										while( string.find( playerName, "#%x%x%x%x%x%x" ) ) do
		...
		if scoreboardDrawn then
			...
			--dxDrawText("Anon-Roam",topX+s(10),y,topX+scoreboardDimensions.width-(10),y+s(20),tocolor ( math.random(255), math.random(255), math.random(255)),fontscale(contentFont, s(0.8)), contentFont,"right","center",true)
		end
	end
end
